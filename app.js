...
app.get("/tweets/:tweetId/", authenticateToken, async (request, response) => {
  ...
  const userFollowsQuery = `SELECT * FROM follower
    INNER JOIN user ON user.user_id=follower.following_user_id
    WHERE follower.follower_user_id=${user_id};`;
...
app.get(
  ...
  async (request, response) => {
    ...
    const { user_id, name } = payload;
    console.log(name, tweetId);
    const getLikedUsersQuery = `SELECT * FROM 
...
app.get(
  ...
  async (request, response) => {
    ...
    const { user_id, name } = payload;
    console.log(name, tweetId);
    const getRepliedUsersQuery = `SELECT * FROM follower INNER JOIN tweet ON 
    tweet.user_id=follower.following_user_id INNER JOIN reply ON reply.tweet_id=tweet.tweet_id
...
app.get("/user/tweets/", authenticateToken, async (request, response) => {
  ...
  const getTweetsDetailsQuery = `SELECT tweet.tweet AS tweet,
            COUNT(DISTINCT(like.like_id)) AS likes,
            COUNT(DISTINCT(reply.reply_id)) AS replies,
            tweet.date_time AS dateTime
        FROM user INNER JOIN tweet ON user.user_id=tweet.user_id INNER JOIN like ON 
             like.tweet_id=tweet.tweet_id INNER JOIN reply ON 
...
const dateTime=new Date().toJSON().substring(0,19).replace("T"," ")
  const postTweetQuery = `INSERT INTO tweet(tweet,user_id,date_time)
    VALUES ('${tweetId}',${user_id},'${dateTime}');`;
  await db.all(postTweetQuery);
